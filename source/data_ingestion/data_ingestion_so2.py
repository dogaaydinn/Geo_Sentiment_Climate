...
logger = setup_logger(
    name="data_ingestion_so2",
    log_file=Path(config["paths"]["logs_dir"]) / "data_ingestion_so2.log",
...
def find_so2_csv_files(raw_dir: Path) -> list[Path]:

    csv_files = list(raw_dir.rglob("*.csv"))
    so2_files = [f for f in csv_files if "so2" in f.name.lower()]
    return so2_files

def process_so2_file(csv_path: Path, metadata: Dict[str, Any], max_rows_read: Optional[int] = None) -> pd.DataFrame:
    file_name = csv_path.name
    logger.info(f"Processing so2 file: {file_name}")
    ...
    logger.info(f"File {file_name} => {rows_count} rows processed for so2")
...
def ingest_so2_data():
    ...
    logger.info("=== Starting so2 ingestion ===")
    metadata = load_metadata(METADATA_PATH)

    so2_files = find_so2_csv_files(RAW_DIR)
    logger.info(f"Found {len(so2_files)} so2 files in raw_dir")

    ingestion_cfg = config.get("ingestion", {})
    max_rows_read = ingestion_cfg.get("max_rows_read", None)

    combined_so2 = pd.DataFrame()
    for path in so2_files:
        df_so2 = process_so2_file(path, metadata, max_rows_read)
        if not df_so2.empty:
            combined_so2 = pd.concat([combined_so2, df_so2], ignore_index=True)
            # Save interim
            try:
                INTERIM_DIR.mkdir(parents=True, exist_ok=True)
                interim_file = INTERIM_DIR / f"interim_{path.name}"
                df_so2.to_csv(interim_file, index=False)
    ...
    if not combined_so2.empty:
        # final deduplicate
        init_shape = combined_so2.shape
        combined_so2.drop_duplicates(inplace=True)
        final_shape = combined_so2.shape
        logger.info(f"Combined so2 dedup: {init_shape} => {final_shape}")

        # save so2_preprocessed
        PROCESSED_DIR.mkdir(parents=True, exist_ok=True)
        out_file = PROCESSED_DIR / "so25_preprocessed.csv"
        combined_so2.to_csv(out_file, index=False)
        logger.info(f"Saved so2 preprocessed => {out_file}")
    else:
        logger.warning("No so2 data combined => skipping final save.")

    # update metadata? e.g. save_metadata(metadata, METADATA_PATH)
    logger.info("=== so2 ingestion completed ===")
